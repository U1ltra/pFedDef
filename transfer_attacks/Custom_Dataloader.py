...
class Custom_Dataloader:
    ...
    def load_batch(self, batch_size, mode="test"):
        ...
        out_x_data = self.x_data[samples].to(device="cuda") #, dtype=torch.float64)
    ...
    def select_single_idx(self, idx):
        x_point = self.x_data[idx:idx+1].to(device="cuda")
        y_point = self.y_data[idx:idx+1].to(device="cuda")
